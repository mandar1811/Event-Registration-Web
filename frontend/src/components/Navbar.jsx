...
function Navbar() {
  ...
  return (
    <nav 
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled ? "bg-white shadow-md py-2" : "bg-white/90 backdrop-blur-sm py-4"}`}
      ref={navRef}
    >
      <div className="w-[90%] mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex space-x-8">
          <Link 
            to="/allEvents" 
            className={`text-gray-600 hover:text-purple-600 relative transition-all duration-300 after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-purple-600 after:scale-x-0 after:origin-left after:transition-transform after:duration-300 hover:after:scale-x-100 ${location.pathname === "/allEvents" ? "text-purple-600 after:scale-x-100" : ""}`}
          >
            Explore Events
          </Link>
          {access_token && (
            <Link 
              to="/profile" 
              className={`text-gray-600 hover:text-purple-600 relative transition-all duration-300 after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-purple-600 after:scale-x-0 after:origin-left after:transition-transform after:duration-300 hover:after:scale-x-100 ${location.pathname === "/profile" ? "text-purple-600 after:scale-x-100" : ""}`}
            >
              My Profile
            </Link>
          )}
      ...
      <div 
        className={`fixed inset-0 bg-black z-40 transition-opacity duration-500 ${isSidebarOpen ? 'opacity-50' : 'opacity-0 pointer-events-none'}`}
        onClick={toggleSidebar}
      />

      <div 
        className={`fixed top-0 right-0 h-full w-72 bg-white shadow-2xl z-50 transform transition-transform duration-500 ease-in-out ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="flex justify-between items-center p-6 border-b">
          <div className="flex items-center space-x-2 group">
            ...
            <span className="font-bold text-lg bg-clip-text text-transparent bg-gradient-to-r from-purple-600 to-indigo-600">EventHub</span>
          </div>
          <button 
            onClick={toggleSidebar}
        ...
        <div className="flex flex-col p-6 space-y-4">
          <Link 
            to="/allEvents"
            className={`py-3 text-gray-600 hover:text-purple-600 border-b hover:pl-2 transition-all duration-300 ${location.pathname === "/allEvents" ? "text-purple-600 font-medium" : ""}`}
            onClick={toggleSidebar}
          >
            Explore Events
          </Link>
          {access_token && (
            <Link 
              to="/profile"
              className={`py-3 text-gray-600 hover:text-purple-600 border-b hover:pl-2 transition-all duration-300 ${location.pathname === "/profile" ? "text-purple-600 font-medium" : ""}`}
              onClick={toggleSidebar}
            >
              My Profile
            </Link>
